...
metal_data_sf <- st_as_sf(metal_sf, coords = c('X', 'Y'), crs = 4269)

st_crs(census_sf)$epsg 
st_crs(metal_sf)$epsg

metal_intersection_withcounties <- census_sf[metal_sf,]
metal_intersection_withcounties2 <- census_sf %>% 
  filter(st_intersects(x = ., y = metal_sf, sparse = FALSE))
```
