...
\newpage
# Introduction 
>Lead is a naturally occurring metal found in the Earth’s crust and is used commonly in many human products. However, its ubiquitous use has caused extensive environmental contamination and public health concerns in many parts of the world. Main sources of lead contamination include mining, smelting, manufacturing, recycling activities, leaded paint, leaded gasoline, and leaded aviation fuel. Over three quarters of global lead consumption is for the manufacturing of lead-acid batteries for motor vehicles (WHO 2019). 
Once ingested, lead spreads through the blood into other parts of the body.  Depending on the level of exposure, lead can adversely affect the nervous system, kidney function, immune system, reproductive and developmental systems and the cardiovascular system (EPA).  One of the biggest health concerns of lead consumption is the neurological effects on children. Children are particularly sensitive to lead exposure and can have side effects such as behavioral problems, learning disabilities, and lower IQ. 
	Currently, the National Ambient Air Quality Standards (NAAQS) for lead are 0.15 micrograms per cubic meter Pb in total suspended particles as a 3-month average (EPA). Air quality monitors are placed throughout the United States in order to measure air lead levels. In our project, we look at lead air quality monitors in the state of Pennsylvania from 2010 to 2020. We were interested in how air lead concentrations vary from 2010 to 2020 across major metropolitan areas in Pennsylvania, where air lead concentration hotspots occur, and 
...
```{r spatial-analysis}
#Data wrangling: Reduce our data to just one record for each county, determining mean and max daily air lead values for 2010, 2015, and 2020
...
ggplot() + 
  ...
  ggtitle("Mean Lead Values Across Pennsylvania")

ggplot() + 
  geom_sf(data=PA_county_2010,aes(fill = meanPOP),alpha=0.3) +
   scale_fill_gradient2(low="blue", mid = "yellow",high="red", guide = "colourbar", breaks = c(10000, 1500000)) +
   labs(fill = "% Pop in Poverty")+
  geom_sf(data=sf_PA_Lead_avg2010, aes(size=meanPb))+
  labs(size="Lead Values")+
  ggtitle("Mean Lead Values Across Pennsylvania")
...
ggplot() + 
  ...
  scale_fill_gradient2(low="blue",high="red", midpoint = 28000, breaks=c(16000,28000, 40000)) + 
...
ggplot() + 
 ...
 scale_fill_gradient2(low="blue",high="red", midpoint = 28000, breaks=c(16000,28000, 40000)) +
  ...
  labs(size="Lead Levels")+
  ggtitle("2010 Max Lead Levels over .15")
...
ggplot() + 
  geom_sf(data=PA_county_2014,aes(fill = meanPCI),alpha=0.3) + 
  scale_fill_gradient2(low="blue",high="red", midpoint = 28000, breaks=c(18000,28000, 40000)) +
  labs(fill="Per Capita Income")+
  geom_sf(data=sf_PA_Lead_high_2015, aes(size=maxPb))+
  labs(size="Lead Levels")+
  ggtitle("2015 Max Lead Levels over .15")
...
ggplot() + 
  geom_sf(data=PA_county_2018,aes(fill = meanPCI),alpha=0.3) + 
  scale_fill_gradient2(low="blue",high="red", midpoint = 35000, breaks=c(20000,35000, 45000)) +
  labs(fill="Per Capita Income")+
  geom_sf(data=sf_PA_Lead_high_2020, aes(size=maxPb))+
  labs(size="Lead Levels")+
  ggtitle("2020 Max Lead Levels over .15")
